...
# Multi-head latent attention (DeepSeek-V2/V3 family: openPangu, DeepSeek-V3, ...).
# Low-rank q (optional) and kv projections with decoupled rope; QK head dim may
# differ from V head dim. Two compute paths: naive (up-project K/V, dtype scores)
# and absorbed (score in latent space, fp32) -- absorbed reads kv_b's raw fp16
# weight, which is why kv_b stays unquantized. Phase 1: full-sequence forward,
# batch 1, no KV cache.
#
# Model-specific behavior (convs, sinks, indexers) hooks in via subclassing:
# hook_q_lora / hook_kv_latent / hook_attn_out (identity here) and extra_kv
# (extra always-visible latent/rope positions, e.g. learned sinks; None here).
# The base takes no model-specific state.
...
class MLAAttention(Module):
    ...
    # -- model-specific hook points (subclass; identity/None here) --
    ...
    def extra_kv(self):
        # -> (latent [n, kv_lora], k_pe [n, rope]) always-visible positions, or None
        return None
    ...
    def forward(self, x, params, out_dtype = None):
        # [seq, hidden] or [1, seq, hidden]; batch 1
        orig_shape = x.shape
